...
import { QTY_BTN, SectionTitle } from "../components/ui/index.jsx";
import { CATEGORIES, STATUS_COLORS, STATUS_ICONS, STATUS_LABELS } from "../lib/constants.js";
  ...
  if (!cart.length) {
    return (
      <div style={{ padding: "80px 20px", textAlign: "center" }}>
        ...
        <h2 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 700 }}>Your cart is empty</h2>
        <p style={{ color: "#9CA3AF", marginTop: 8 }}>Add some homeopathic medicines to get started!</p>
      </div>
    );
  }

  return (
    <div style={{ padding: "clamp(20px,5vw,40px) clamp(16px,5vw,80px)", display: "grid", gridTemplateColumns: "1fr min(340px,100%)", gap: 40, alignItems: "start" }}>
      <div>
        <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(20px,3vw,26px)", fontWeight: 800, marginBottom: 22 }}>Shopping Cart</h1>
        {cart.map((item) => (
      ...
      <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: "0 2px 8px rgba(0,0,0,.08)", position: "sticky", top: 80 }}>
        <h2 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 800, marginBottom: 20 }}>Order Summary</h2>
        {cart.map((i) => (
...
export function WishlistPage({ products, wishlist, onAdd, onWishlist }) {
  const { default: ProductCard } = { default: require("../components/shop/ProductCard.jsx") };
  ...
  return (
    <div style={{ padding: "clamp(20px,5vw,40px) clamp(16px,5vw,80px)", minHeight: "60vh" }}>
      <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(22px,3vw,28px)", fontWeight: 800, marginBottom: 4 }}>My Wishlist</h1>
...
export function OrderSuccess({ order, onContinue, onViewOrders }) {
  return (
    <div style={{ padding: "clamp(40px,8vw,80px) clamp(20px,5vw,40px)", textAlign: "center", maxWidth: 560, margin: "0 auto" }}>
      ...
      <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 900, fontSize: "clamp(24px,3vw,30px)", marginBottom: 10 }}>Order Placed!</h1>
      <p style={{ color: "#6B7280", fontSize: 15, lineHeight: 1.7, marginBottom: 28 }}>
        Thank you! Your homeopathic medicines will be delivered soon. You'll receive an SMS notification at each step.
      </p>
      <div style={{ background: "#F0FDF4", border: "1px solid #86EFAC", borderRadius: 16, padding: 22, marginBottom: 28 }}>
        ...
        <div style={{ fontSize: 12, color: "#6B7280", marginTop: 8 }}>📱 SMS notification sent to your registered number</div>
      </div>
      <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
        <button onClick={onViewOrders} style={{ background: "#15803D", color: "#fff", border: "none", borderRadius: 10, padding: "12px 24px", fontWeight: 700, cursor: "pointer" }}>Track My Order</button>
        <button onClick={onContinue}   style={{ background: "#F3F4F6", color: "#374151", border: "none", borderRadius: 10, padding: "12px 24px", fontWeight: 700, cursor: "pointer" }}>Continue Shopping</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ background: "linear-gradient(135deg,#0d2b0d,#1a4a1a)", padding: "clamp(40px,8vw,80px) clamp(20px,5vw,80px) 60px", textAlign: "center" }}>
        ...
        <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(28px,4vw,44px)", fontWeight: 900, color: "#fff", margin: "0 0 14px" }}>About NaturaHeal</h1>
      ...
      <div style={{ padding: "clamp(30px,5vw,60px) clamp(20px,5vw,80px)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "center" }}>
        <div>
          <h2 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(22px,3vw,30px)", fontWeight: 800, marginBottom: 16 }}>Our Story</h2>
        ...
        <div style={{ borderRadius: 20, overflow: "hidden", height: 320 }}>
          <img src="https://images.unsplash.com/photo-1512069772995-ec65ed45afd6?w=600&auto=format&fit=crop&q=80" alt="Natural herbs" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        </div>
      </div>
      <div style={{ background: "#F9FAFB", padding: "44px clamp(20px,5vw,80px)", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 22, textAlign: "center" }}>
...
export function ContactPage() {
  ...
  return (
    <div style={{ padding: "clamp(30px,5vw,60px) clamp(20px,5vw,80px)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "start" }}>
      <div>
        <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(26px,3vw,38px)", fontWeight: 900, marginBottom: 14 }}>Get in Touch</h1>
        <p style={{ color: "#6B7280", marginBottom: 32, lineHeight: 1.7 }}>Questions about our homeopathic medicines? Our team is here to help.</p>
        {[
          ...
          <div key={label} style={{ display: "flex", gap: 14, marginBottom: 22 }}>
            <div style={{ width: 44, height: 44, background: "#DCFCE7", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>{icon}</div>
            <div>
      ...
      <div style={{ background: "#fff", borderRadius: 20, padding: 32, boxShadow: "0 4px 16px rgba(0,0,0,.08)" }}>
        {sent ? (
          <div style={{ textAlign: "center", padding: "40px 0" }}>
            <div style={{ fontSize: 48, marginBottom: 14 }}>✅</div>
            <h3 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 700 }}>Message Sent!</h3>
            <p style={{ color: "#6B7280", marginTop: 8 }}>We'll get back to you within 24 hours.</p>
          </div>
        ) : (
          <>
            <h2 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 800, marginBottom: 22 }}>Send a Message</h2>
            {[["Name", "name", "Your name", "text"], ["Email", "email", "your@email.com", "email"], ["Phone", "phone", "+91 XXXXX XXXXX", "tel"]].map(([l, k, ph, t]) => (
            ...
            <div style={{ marginBottom: 22 }}>
              ...
              <textarea placeholder="How can we help?" value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} rows={4}
